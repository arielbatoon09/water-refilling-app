...
const handleBookNow = async () => {
  ...
  if (response.status === 200) {
    ...
    FormData.value.delivery_type = 'Select delivery type';
    FormData.value.mop = 'Select mode of payment';
...
<template>
  <div class="modal-box">
    ...
    <div class="w-full">
      ...
      <!-- 1st Slide Fields -->
      <div v-if="slideState === 0" class="slide-1">
        <!-- Gallon Sizes -->
      ...
      <div v-if="slideState === 1" class="slide-2">
        ...
        <div class="space-y-1 mt-4">
          ...
          <select v-model="FormData.mop" class="select select-bordered w-full">
        ...
        <!-- Delivery Date -->
        <div class="space-y-1 mt-4">
          <span class="text-gray-700 font-medium">Date of Delivery</span>
          <input v-model="FormData.delivery_date" :min="currentDate" type="date" class="input input-bordered w-full" />
        </div>
      </div>

      <div class="space-y-2 mt-6">
        <button v-if="slideState === 0" @click="handleSlideState"
          class="btn primary-btn-bg text-white w-full">Next</button>
        <button v-if="slideState === 1" @click="handleBookNow" class="btn primary-btn-bg text-white w-full">Book
          Now</button>
        <button v-if="slideState === 1" @click="handleSlideState" class="btn w-full">Back</button>
      </div>
    </div>
  </div>
